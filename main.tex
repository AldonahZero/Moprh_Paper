...
\vspace{0.2em}
\parbox{\columnwidth}{\scriptsize\raggedright\textit{Note.} \cmark: explicit support for test-case migration; \partmark: task-limited support in our setting, as explained below; \xmark: not supported.}
\end{table}
The \partmark entries indicate that a tool may handle the corresponding SQL category in some inputs, but the support does not directly meet the requirements of our test-case migration task.
The reason is that a fuzzing seed is not just a translated statement: it is an ordered sequence in which DDL creates schema state, DML populates data state, and later DQL statements depend on both states while still needing to remain parsable and mutable by the downstream fuzzer.
RISE~\cite{xie2026rise}, CrackSQL~\cite{zhou2025cracking}, and QTRAN~\cite{lin2025qtran} can cover parts of DDL/DML/DQL in their intended settings, such as \texttt{SELECT}-query translation, stored-procedure translation, query-snippet translation, or metamorphic query-pair transfer.
However, these scoped settings do not provide an end-to-end guarantee that a full seed sequence will preserve schema/data dependencies across statements.
\sedar{}~\cite{fu2024sedar} is closer to our setting because it transfers DBMS test cases, but when DDL/DML fragments exceed the downstream fuzzer grammar, its unparsable-commenting strategy can hide those fragments and remove mutation-relevant structure.
Thus, \partmark denotes support that exists in restricted scenarios but is limited for dependency-preserving, fuzzer-compatible test-case migration.
...
In scope, RISE operates on one query or stored-procedure input at a time, whereas an AQT spans an ordered, multi-statement test case and therefore captures cross-statement read/write dependencies $\Delta$ and an evolving schema context $\mathcal{D}$ that do not arise in isolated translation inputs.
