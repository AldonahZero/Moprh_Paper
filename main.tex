...
\tool{} builds on these lines of work but targets a different problem: structure-preserving transfer of whole SQL test cases into high-quality fuzzing seeds.
...
In contrast, \tool{} uses the LLM only within an AQT-guided instantiation process, where schema dependencies, nested structures, and clause-level relationships are explicit and can be repaired locally without discarding the surrounding test-case structure.
...
In particular, although the AQT may appear to resemble the query-reduction (``skeleton'') step of RISE~\cite{xie2026rise}, the two constructs are formally disjoint: RISE reduces a \emph{single} SELECT statement to a minimal, dialect-bearing subquery and encodes the dialect as a statement-local, schema-free AST rewrite rule $\langle \mathit{Pat}_{src}, \mathit{Pat}_{tar} \rangle$, whereas an AQT is a five-tuple over an \emph{ordered, multi-statement} test case that additionally carries cross-statement read/write dependencies $\Delta$ and their induced graph $\mathcal{G}_{tc}$, an evolving schema/type/constraint context $\mathcal{D}$ propagated across statements, and a structural hash $\mathcal{H}$ for corpus-level deduplication---none of which is definable for a single-statement reducer.
Consequently, the dialect-token abstraction shared by both methods is, in \tool{}, merely the per-statement field $S^{i}_{norm}$ of one element of the tuple rather than the representation itself, so the rewrite rule that constitutes RISE's central artifact corresponds only to a \emph{local} AQT-node instantiation operator in our design.
...
The artifact and experimental data are available in an anonymous archive with a DOI at \url{https://doi.org/10.5281/zenodo.19251660}.
