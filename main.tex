...
(2) Second, even when translation produces target-dialect SQL, the result may exceed the grammar accepted by downstream fuzzers. Existing methods often handle such incompatibilities by mechanically discarding or commenting out unsupported grammar structures, causing the loss of valuable SQL features and testing logic embedded in the original test cases. As a result, although the transferred queries may remain syntactically valid, they suffer from semantic degradation, limiting coverage of the target DBMS's execution paths and reducing the effectiveness of stress-testing the query optimizer.
...
Our core idea is to \textbf{reformulate SQL transfer from direct SQL-text translation into test-case-level AQT-Guided Dialect Instantiation}.
Instead of translating each SQL statement or raw test case as text, \tool{} first extracts an AQT that explicitly captures schema dependencies, nested structures, and clause-level relationships.
...
\ForEach{$id$ \textbf{in} $Order$}{
    ...
    \ForEach{$\tau$ \textbf{in} $\textsc{AbstractTokens}(S)$}{
        ...
        \If{$conf < \delta$}{
            $rule \leftarrow \textsc{Generator}(\tau, D_t)$ \label{line:alg2:generate} \tcp*{Candidate local rule}
        }
        ...
        \If{$conf < \delta$}{
            $HKB.\textsc{Cache}(\tau, rule)$ \label{line:alg2:cache_rule} \tcp*{Cache parser-validated rule}
        }
...
Each abstract token $\tau$ is instantiated using a local mapping retrieved from the Hybrid Knowledge Base (Line~\ref{line:alg2:retrieve}). When retrieval confidence falls below a threshold $\delta$, the Rule Generator only synthesizes a candidate local rule from the AST context and few-shot examples (Line~\ref{line:alg2:generate}). This candidate is first applied by the translator to instantiate the token and then checked by the target DBMS parser before it can enter the Hybrid KB (Lines~\ref{line:alg2:translate}--\ref{line:alg2:cache_rule}).
...
% If the retrieval confidence falls below a threshold $\delta$, a rule generator synthesizes a candidate local rule, which is cached only after target-parser validation succeeds.

To ensure syntactic correctness under semantic constraints, each generated fragment is validated by the target database parser. If a parsing error occurs, an iterative repair loop repairs the fragment and its local rule using the error message and structural context while preserving the seed's semantic-rich structure (Line~\ref{line:alg2:fix}). Only after parser validation succeeds does \tool{} cache the corresponding generated rule; unverified candidates are discarded rather than reused. This mechanism avoids degenerate fixes such as clause removal and preserves testing-relevant SQL features.
...
% \textit{Rule Generator:} This skill synthesizes a candidate local rule under low retrieval confidence and caches it only after target-parser validation succeeds.
...
The algorithm first performs a topological sort on the structure graph $\mathcal{G}$ to determine a bottom-up execution sequence (Line~\ref{line:alg2:sort}), ensuring nested subqueries are resolved before their parents. For each segment, the planner recursively substitutes instantiated child fragments (Lines~\ref{line:alg2:substitute_start}). Each abstract token $\tau$ is instantiated using rules retrieved from the \texttt{Hybrid KB} (Line~\ref{line:alg2:retrieve}), or, if confidence is low, a generator creates a candidate local rule (Line~\ref{line:alg2:generate}). The token is translated into target-compliant syntax (Line~\ref{line:alg2:translate}) and iteratively parsed and fixed (Line~\ref{line:alg2:fix}); generated rules are cached only after parser validation succeeds. Compliant fragments are then cached and assembled into the final seed $Q_{target}$ (Lines~\ref{line:alg2:cache}--\ref{line:alg2:assemble}).
...
General-purpose SQL dialect translators, such as SQLGlot~\cite{sqlglot} and jOOQ~\cite{jooq}, convert SQL syntax across DBMS dialects for query migration and application compatibility. They provide useful mappings for individual statements, but they do not treat a fuzzing seed as a whole test case with DDL/DML dependencies, schema constraints, and downstream mutation requirements. Recent work RISE~\cite{xie2026rise} further studies semantic-equivalent SELECT translation across RDBMSs by decomposing a SELECT statement into skeletons, isolating dialect-specific constructs, and using LLMs to infer translation rules. However, this statement-level design does not model cross-statement dependencies and does not ensure that the translated result is acceptable to a fuzzer's internal grammar.
Recent LLM-based methods improve DBMS seed generation: CrackSQL~\cite{zhou2025cracking} and Fuzz4All~\cite{xia2024fuzz4all} improve seed variety, while \sedar{}~\cite{fu2024sedar}, ShQveL~\cite{zhong2025testing}, and QTRAN~\cite{lin2025qtran} explore cross-dialect transfer, fragment synthesis, and oracle extension.
...
AQTs expose schema dependencies, nested structures, and clause-level relationships as an explicit structural space; target-dialect rules are retrieved or generated within this space; and parser/executor feedback performs local structural repair rather than deleting invalid constructs. Thus, \tool{} targets test-case-level seed transfer rather than statement-level translation, preserving test-case structure while repairing fuzzer-incompatible constructs.
