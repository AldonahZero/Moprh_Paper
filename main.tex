...
\title{\tool{}: Structure-Aware Cross-Dialect SQL Transfer for Constructing High-Quality Seeds in DBMS Fuzzing}
...
First, \tool{} builds an Abstract Query Template (AQT) with dialect-independent structure extraction, preserving query structure and semantic-rich dependencies.
...
(1) First, end-to-end SQL translation may introduce syntax or semantic errors due to the inherent hallucination of LLMs or their limited knowledge of target dialects. Such errors often break schema references, type constraints, or function semantics, making the transferred seeds fail during execution on the target DBMS. 
...
Our core idea is to \textbf{reformulate SQL transfer from LLM-driven text translation into constrained instantiation over Abstract Query Templates (AQTs)}.
Instead of asking an LLM to translate raw SQL end-to-end, \tool{} first extracts an AQT that explicitly captures schema dependencies, nested structures, and clause-level relationships.
It then instantiates the AQT into target-dialect SQL using explicit transfer rules and target-side feedback, with the LLM used only for bounded rule generation or local repair under structural constraints.
...
\begin{figure*}[t]
  ...
  \caption{\textbf{Overview of \tool{}.} \tool{} transfers source test cases through structure-preserving abstraction, rule-guided dialect instantiation, and feedback-driven adaptive fuzzing.}
  \label{fig:workflow}
  % \vspace{-0.5cm}
\end{figure*}
...
Unlike LLM-centric text translation, \tool{} treats transfer as a constrained instantiation process over AQTs.
It first abstracts source SQL into a structure that records schema dependencies, nested query structures, and clause-level relationships; then it recursively instantiates abstract nodes with rules retrieved or generated under target-dialect constraints.
...
\item
    ...
    An AQT is defined as a 4-tuple:
    $$\mathcal{T} = \langle S_{norm}, \mathcal{D}, \mathcal{G}, \mathcal{H} \rangle$$
    \begin{itemize}
...
\tool{} first parses the source query $Q_{src}$ into a schema-bound AST (Line~\ref{line:alg1:parse}); a boundary is an AST node whose subtree has an independent SQL scope, such as a CTE body, nested subquery (e.g., \texttt{IN (SELECT ...)}), or standalone query block.
During \textsc{Traverse}, \textsc{IsBoundary} extracts each such subtree as a separate AQT unit, inserts a pointer in the parent segment, and records the edge $(pid,id)$ in $\mathcal{G}$, preserving the original scope and nesting relation (Lines~\ref{line:alg1:addedge}--\ref{line:alg1:boundary_end}).
...
All extracted segments are recorded in $Segs$ (Line~\ref{line:alg1:segs}). Finally, \tool{} computes a normalized skeleton $S_{norm}$, a structural hash $\mathcal{H}$, and schema context $\mathcal{D}$ to assemble the AQT (Lines~\ref{line:alg1:norm}--\ref{line:alg1:return}). This representation enables SQL queries to be manipulated as structured graph-based objects rather than flat strings.
...
\Output{AQT $\mathcal{T} = \langle S_{norm}, \mathcal{D}, \mathcal{G}, \mathcal{H} \rangle$}
...
$\mathcal{H} \leftarrow \textsc{SHA256}(S_{norm})$ \tcp*{Structural hash}
$\mathcal{D} \leftarrow \textsc{ExtractSchema}(\mathcal{S}, Q_{src})$ \tcp*{Schema context}
\Return $\langle S_{norm}, \mathcal{D}, \mathcal{G}, \mathcal{H} \rangle$\; \label{line:alg1:return}
...
\Fn{\textsc{Traverse}($node$, $pid$)}{ \label{line:alg1:traverse_start}
    ...
    \ForEach{$c$ \textbf{in} $node.children$}{
        \uIf{\textsc{IsBoundary}($c$)}{ \label{line:alg1:boundary_start}
            $cid \leftarrow \textsc{Traverse}(c, id)$ \tcp*{CTE/subquery/query block}
            $tokens.\textsc{Append}(\texttt{<Ptr:}cid\texttt{>})$\; \label{line:alg1:boundary_end}
        }
...
The step aims to instantiate AQTs into a syntactically valid query that adheres to the target grammar while preserving semantic-rich structures from the source seed.
The challenge is preserving target-side semantic correctness in recursive SQL substitution.
...
\Input{AQT $\mathcal{T} = \langle S_{norm}, \mathcal{D}, \mathcal{G}, \mathcal{H} \rangle$; Target dialect $D_t$; Hybrid KB $HKB$; Threshold $\delta$}
...
\ForEach{$id$ \textbf{in} $Order$}{
    ...
    \ForEach{$c$ \textbf{in} $\mathcal{G}.children(id)$}{ \label{line:alg2:substitute_start}
        $S \leftarrow \textsc{Substitute}(S, c, Fragments[c])$ \label{line:alg2:substitute_end} \;
    }
    \ForEach{$\tau$ \textbf{in} $\textsc{AbstractTokens}(S)$}{
        ...
        \If{$conf < \delta$}{
            $rule \leftarrow \textsc{Generator}(\tau, D_t)$ \label{line:alg2:generate} \tcp*{Candidate rule}
        }
        $frag \leftarrow \textsc{Translator}(\tau, rule, \mathcal{D})$ \label{line:alg2:translate} \tcp*{Apply rule}
        \While{$err \leftarrow \textsc{Parse}(frag, D_t)$}{
            $(frag, rule) \leftarrow \textsc{Fixer}(frag, rule, err)$ \label{line:alg2:fix} \;
        }
        \If{$conf < \delta$}{
            $HKB.\textsc{Cache}(\tau, rule)$ \label{line:alg2:cache_rule} \tcp*{Verified rule only}
        }
...
Each abstract token $\tau$ is instantiated using rules retrieved from the Hybrid Knowledge Base (Line~\ref{line:alg2:retrieve}). When retrieval confidence falls below a threshold $\delta$, a rule generator synthesizes a candidate rule from the AST context and few-shot examples (Line~\ref{line:alg2:generate}). The candidate is applied by the translator and validated by the target DBMS parser before it is cached (Lines~\ref{line:alg2:translate}--\ref{line:alg2:cache_rule}).
...
% If the retrieval confidence falls below a threshold $\delta$, a rule generator synthesizes a candidate rule, which is cached only after target-parser validation succeeds.

To ensure syntactic correctness under semantic constraints, each generated fragment is validated by the target database parser. If a parsing error occurs, an iterative repair loop repairs the fragment and its rule using the error message and structural context while preserving the seed's semantic-rich structure (Line~\ref{line:alg2:fix}). Only successfully validated rules are cached; unverified rules are discarded. This mechanism avoids degenerate fixes such as clause removal and preserves testing-relevant SQL features.
...
\textit{Rule Generation:} This skill is activated when retrieval confidence for a given feature falls below a predefined threshold. It uses the LLM's zero-shot reasoning, AST context, and few-shot examples to synthesize a candidate transformation rule. The candidate is cached in the Hybrid KB only after target-parser validation succeeds.

\begin{figure}[b]
    \centering
    \includegraphics[scale=1.0, width=1\linewidth]{media/rule.pdf}
    \caption{RAG-enhanced rule-guided dialect instantiation.
...
% \textit{Rule Generator:} This skill synthesizes a candidate rule under low retrieval confidence and caches it only after target-parser validation succeeds.
...
When an error signal is received (e.g., ``Syntax error near SELECT''), \tool{} identifies the corresponding node in the AQT and applies a set of \textit{Sanitization Rules}. If no rule matches, the LLM \textit{Error Fixer} skill is invoked~\cite{fan2023automated} to locally rewrite the problematic structure into a compliant form. For example, a rejected window function syntax may be transformed into a standard aggregation form when possible.
...
% The correlation between semantic correctness, coverage, and bug discovery is evident. In the raw baseline ($S^-$), queries often terminate early during parsing due to syntax errors. By ensuring high semantic correctness (RQ2), \tool{} allows the fuzzer to bypass early rejection checks, exercise deeper logic within the optimizer and executor, and ultimately trigger more critical vulnerabilities.
% The correlation between testing strategy, coverage, and bug discovery is evident. 
Unlike \sedar{}, which sacrifices semantic depth through destructive sanitization, \tool{} employs structure-aware transfer to retain the full logical complexity of source queries. By ensuring high semantic correctness (RQ2), \tool{} enables fuzzers to bypass early engine rejections, exercise deeper optimizer and executor logic, and trigger critical vulnerabilities.
...
In this section, we evaluate the effectiveness of \tool{} by comparing it with untranslated initial seeds, off-the-shelf SQL dialect translators, recent SQL translation systems, and \sedar{}.
We evaluate in two aspects: the validity of translated seeds and the semantic richness of SQL features preserved in the translated test cases.
We emphasize that this evaluation targets fuzzing seed quality rather than cross-DBMS result equivalence: valid seeds must execute on the target DBMS, and rich seeds should retain diverse SQL features for subsequent mutation.
...
As shown in Table~\ref{tab:validity}, directly executing source seeds without translation yields low validity in the target DBMSs. 
For instance, the semantic correctness ratio is only 0.097 on Virtuoso, while the highest ratio of No translation is only 0.284 on DuckDB. 
This shows that foreign SQL dialects are largely ineffective as fuzzing seeds without explicit adaptation.
Off-the-shelf translators and recent SQL translation systems improve syntactic acceptance to some extent, but they still leave a substantial semantic gap.
For example, on DuckDB, SQLGlot, CrackSQL, and RISE achieve syntactic correctness of at least 0.738, yet their semantic correctness remains no higher than 0.314.
Although \sedar{} improves semantic correctness, it still suffers from semantic hallucinations and destructive sanitization. 
In contrast, \tool{} achieves the best result in every syntactic and semantic-correctness column, with \synatxratio{} higher syntactic correctness and \translateratio{} higher semantic correctness than \sedar{} on the tested DBMSs.
On \duckdb{}, for example, \tool{} improves semantic correctness from \sedar{}'s 0.365 to 0.748 by preserving target-compatible schema, type, and function semantics during transfer.
% First, we analyze the semantic correctness of the generated seeds. As shown in Table~\ref{tab:validity}, raw seeds ($S^-$) exhibit low transferability, with semantic correctness ratios dropping as low as 9.7\% on Virtuoso. This confirms that direct execution of foreign SQL dialects is ineffective due to syntax incompatibilities.

% ============================================================
% Table 5: Validity Statistics (RQ3) 
% ============================================================
\begin{table}[ht]
\centering % <--- 关键：居中对齐
\caption{Comparison of syntactic and semantic correctness across target DBMSs. Syn. and Sem. denote syntactic correctness and semantic correctness, respectively. A dash indicates that the tool does not support translation to the target dialect. The best result in each column is highlighted in bold. The last row reports the relative improvement of \tool{} over \sedar{}.}
...
The semantic-correctness improvement is particularly pronounced on DBMSs with larger dialect gaps. On Virtuoso, \tool{} achieves a semantic correctness of 0.629, which represents a 272\% improvement over \sedar{}. Consistently, on MonetDB, the semantic correctness reaches 0.776, effectively doubling the performance of \sedar{} with a 100\% increase. Even on DuckDB, where \sedar{} performs relatively better, \tool{} still provides a 105\% boost in semantic correctness. 
...
As summarized in Table~\ref{tab:diversity}, \tool{} demonstrates a consistent ability to maintain a higher degree of semantic richness than all applicable baselines. 
...
In contrast, \sedar{} retains 625 functions, 146 data types, and 872 keywords. 
In other words, compared to \sedar{}, \tool{} covers 411 more functions, 34 more data types, and 496 more keywords, corresponding to aggregate improvements of 66\%, 23\%, and 57\%, respectively.
% Compared with No translation, this is a substantial net increment of +633 functions, +65 data types, and +616 keywords.
...
\begin{table}[ht]
\centering % <--- 让表格居中
\caption{Statistics of valid SQL features in the transferred seeds. F, DT, and K denote functions, data types, and keywords, respectively. A dash indicates that the tool does not support translation to the target dialect. The best result in each column is highlighted in bold. The last row reports percentage improvement of \tool{} over \sedar{}.}
\label{tab:diversity}
{\footnotesize
...
% \tool{} exhibits a superior capability to preserve structural richness. In terms of functions, \tool{} covers a total of \textbf{1,036} distinct functions across four DBMSs, compared to 625 for \sedar{}. This represents 411 more functions than \sedar{}. A notable improvement is also seen in DuckDB, where \tool{} successfully transfers 456 unique functions and 476 keywords, significantly higher than \sedar{}'s 214 functions and 340 keywords.
...
\begin{answerbox}
\textbf{Answer to RQ3:} \tool{} achieves the strongest correctness results across all tested translation methods and DBMSs. It improves semantic correctness by up to 272\% over \sedar{} and preserves 1,036 functions, 180 data types, and 1,368 keywords, exceeding \sedar{} by 411 (+66\%), 34 (+23\%), and 496 (+57\%), respectively. By eliminating semantic hallucinations and employing constructive instantiation, \tool{} ensures generated seeds are both executable and rich in testing logic.
\end{answerbox}
...
Moreover, with \textit{Rule-Guided RAG Agent}, $\tool{}_{\textit{rag}}$ further enhances syntactic correctness to 87\%, and increases semantic correctness to 49\% (+21\%) by grounding generation in retrieved dialect rules.
...
Without adaptive repair, \tooldisfuzzer{} suffers high parser-stage rejection. $\tool{}_{\textit{full}}$ repairs such errors, recovers invalid seeds, and lets fuzzers explore deeper target states. 
...
Table~\ref{tab:manual_effort} summarizes the extension effort across six DBMSs, including two additional targets, SQLServer and OpenTenBase.
Overall, only 3.2\% (140/4,323) of transfer rules require manual correction.
Each correction typically takes 1--3 minutes for an engineer with DBMS expertise; for example, adapting \tool{} to SQLServer requires only three manual corrections.
...
We further validate extensibility on SQLServer, a commercial DBMS with vendor-specific grammars, and OpenTenBase, a distributed DBMS with distributed-specific SQL features.
\tool{} generates 233 rules for SQLServer and 878 rules for OpenTenBase, achieving syntactic correctness of 96.6\% and 98.6\%, and semantic correctness of 71.5\% and 72.7\%, respectively.
...
Beyond test case translation, \tool{} can migrate SQLancer-style oracles~\cite{sqlancer} or compare equivalent cross-DBMS queries, enabling future detection of logic and semantic bugs.
...
Unlike these works, \tool{} addresses the upstream challenge of \textit{seed acquisition} by producing high-quality cross-dialect seeds for complex scenarios and emerging DBMS engines~\cite{gao2023comprehensive}.

\subsection{LLM-Based Seed Generation}
Recent LLM-based methods improve DBMS seed generation: CrackSQL~\cite{zhou2025cracking}, RISE~\cite{xie2026rise}, and Fuzz4All~\cite{xia2024fuzz4all} improve seed variety, while \sedar{}~\cite{fu2024sedar}, ShQveL~\cite{zhong2025testing}, and QTRAN~\cite{lin2025qtran} explore cross-dialect transfer, fragment synthesis, and oracle extension.
\sedar{} is the closest work to \tool{} because it also transfers existing DBMS test cases into fuzzing seeds.
However, \sedar{} remains an LLM-centric SQL text translation framework: it feeds schema information and raw SQL to an LLM and then relies on destructive sanitization, such as commenting out unparsable fragments, to satisfy fuzzer compatibility.
In contrast, \tool{} reformulates transfer as AQT-constrained instantiation.
AQTs expose schema dependencies, nested structures, and clause-level relationships as an explicit structural space; target-dialect rules are retrieved or generated within this space; and parser/executor feedback performs local structural repair rather than deleting invalid constructs.
This design difference leads to stronger empirical results: \tool{} achieves 0.629--0.776 semantic correctness across four DBMSs, compared with 0.169--0.388 for \sedar{}, and further covers 14\% more branches and finds 16 more bugs than \sedar{} in 24 hours.
...
% Extensive evaluations across four distinct DBMS architectures demonstrate that \tool{} significantly outperforms state-of-the-art baselines, improving semantic correctness by up to \textbf{272\%} and achieving superior code coverage. Furthermore, its practical efficacy is evidenced by the discovery of \bugcnt{} previously unknown bugs, covering critical components from the optimizer to the storage engine.
...
We propose \tool{}, an automated fuzzing framework that extracts dialect-agnostic AQTs, instantiates them with a rule-guided RAG agent, and applies adaptive sanitization for cross-dialect DBMS fuzzing. Across four DBMSs, \tool{} improves semantic correctness by up to 272\%, achieves higher coverage than \sedar{}, and detects \bugcnt{} unknown bugs. Future work will extend this transfer paradigm to non-relational systems.

\noindent\textbf{Data.} \url{https://doi.org/10.5281/zenodo.19251660}. 
