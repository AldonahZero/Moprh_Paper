...
Second, \tool{} performs AQT-Guided Dialect Instantiation to construct a target DBMS query under target-dialect constraints.
...
Our core idea is to \textbf{reformulate SQL transfer from LLM-driven text translation into AQT-Guided Dialect Instantiation}.
...
Unlike statement-level translators such as RISE, \tool{} models full test cases, including nested structures, schema dependencies, and clause-level relationships; mapping rules are only local operators for instantiating AQT nodes, rather than the core translation paradigm.
...
(2) Second, \tool{} performs AQT-Guided Dialect Instantiation, recursively instantiating AQT nodes into dialect-specific subtrees under target-dialect constraints using a continuously updated Hybrid Knowledge Base (Hybrid KB) and generating local mappings with LLMs only when necessary.
...
%     \item \textit{AQT-Guided Recursive Dialect Instantiation.} \tool{} recursively replaces AQT nodes with dialect-specific subtrees using mappings from a Hybrid Knowledge Base, which is continuously updated during translation. 
...
%         AQT-Guided Recursive Dialect Instantiation
...
%         \tool{} recursively replaces AQT nodes with dialect-specific subtrees using mappings retrieved from a Hybrid Knowledge Base. The knowledge base, storing target dialect grammar and function mappings, is continuously updated with newly generated mappings. Each node is either mapped to an existing mapping or triggers mapping generation if no mapping exists, ensuring deterministic replacements that preserve query semantics and prevent LLM-induced errors.

%         \textit{2. AQT-Guided Recursive Dialect Instantiation.} 
...
\begin{figure*}[t]
  ...
  \caption{\textbf{Overview of \tool{}.} \tool{} transfers source test cases through structure-preserving abstraction, AQT-Guided Dialect Instantiation, and feedback-driven adaptive fuzzing.}
  \label{fig:workflow}
  % \vspace{-0.5cm}
\end{figure*}
...
% \tool{} implements an \textit{Abstraction-then-Instantiation} paradigm that utilizes static analysis to extract dialect-agnostic Abstract Query Templates (AQT) preserving logical topology, and subsequently employs AQT-guided recursive instantiation to map these templates to the target dialect for robust and non-destructive logic adaptation.
% We find that decoupling invariant query structure and semantics from dialect-specific syntax facilitates cross-dialect SQL transfer, rather than directly translating SQL text across DBMSs.
% Key challenges in achieving this include extracting query logic, preserving semantics in translation, and adapting queries for fuzzers.
...
% --- 插入跨双栏的工作流图 ---
  ...
  %感觉有点多了，可以精简描述
  % \Description{A workflow diagram illustrating Morph's three-phase architecture: Syntax-Aware Query Abstraction extracts AQTs from source SQL, AQT-Guided Recursive Instantiation generates target-compatible seeds using an LLM agent, and Feedback-Driven Adaptive Sanitization refines seeds based on fuzzer feedback.}
...
%     \item \textbf{AQT-Guided Dialect Instantiation:} 
...
This step instantiates AQTs into syntactically valid target queries while preserving semantic-rich structures from the source seed.
The challenge is preserving target-side semantic correctness during recursive AQT-node instantiation.
...
\small
\caption{AQT-Guided Recursive Dialect Instantiation}
...
To guarantee that the generated queries remain semantically consistent with the target dialect during recursive construction, the system employs an AQT-Guided Dialect Instantiation process as formalized in Algorithm~\ref{alg:translation}. The algorithm first performs a topological sort on the structure graph $\mathcal{G}$ to determine a bottom-up execution sequence (Line~\ref{line:alg2:sort}), ensuring that nested subqueries are resolved before their parents. For each segment, the algorithm recursively substitutes instantiated child fragments into the current structure (Lines~\ref{line:alg2:substitute_start}--\ref{line:alg2:substitute_end}).
Each abstract token $\tau$ is instantiated using a local mapping retrieved from the Hybrid Knowledge Base (Line~\ref{line:alg2:retrieve}). When retrieval confidence falls below a threshold $\delta$, the Rule Generator synthesizes a candidate rule from the AST context and few-shot examples (Line~\ref{line:alg2:generate}). The candidate is applied by the translator and validated by the target DBMS parser before it is cached (Lines~\ref{line:alg2:translate}--\ref{line:alg2:cache_rule}).
...
To ensure syntactic correctness under semantic constraints, each generated fragment is validated by the target database parser. If a parsing error occurs, an iterative repair loop repairs the fragment and its local rule using the error message and structural context while preserving the seed's semantic-rich structure (Line~\ref{line:alg2:fix}). Only successfully validated rules are cached; unverified candidates are discarded. This mechanism avoids degenerate fixes such as clause removal and preserves testing-relevant SQL features.
...
\textit{Rule Generator:} This skill is activated when retrieval confidence for a given feature falls below a predefined threshold. It uses the LLM's zero-shot reasoning, AST context, and few-shot examples to synthesize a candidate local rule. The candidate is cached in the Hybrid KB only after target-parser validation succeeds.
...
% +算法描述或换成图来解释
% ============================================================
% Algorithm 2: AQT-Guided Dialect Instantiation (Compact)
% ============================================================
\begin{algorithm}[htbp]
\small
\caption{AQT-Guided Recursive Dialect Instantiation}
...
To guarantee that the generated queries remain semantically consistent with the target dialect during recursive construction, \tool{} employs an AQT-Guided Dialect Instantiation process as formalized in Algorithm~\ref{alg:translation}. 
...
\item \textbf{RQ4:} What is the contribution of each component in \tool{} to the overall performance?
    % , including the \textit{Syntax-Aware Query Abstraction}, the \textit{AQT-Guided Dialect Instantiation}, and the \textit{Feedback-Driven Adaptive Sanitizer}, 
\end{itemize}
We first describe the experimental setup, and then answer RQ1--RQ4 in the following four subsections.

    % \item \textbf{RQ4:} How do the individual components of \tool{}—specifically the \textit{Syntax-Aware Query Abstraction}, the \textit{AQT-Guided Dialect Instantiation}, and the \textit{Feedback-Driven Adaptive Sanitizer}—contribute to the overall performance?
...
\begin{table}[ht]
\centering % <--- 关键：居中对齐
\caption{Syntactic and semantic correctness across target DBMSs. Syn. and Sem. denote syntactic and semantic correctness; ``-'' denotes unsupported dialects. Bold marks the best result; the last row reports improvement over \sedar{}.}
...
These gains confirm that AQT-Guided Dialect Instantiation bridges parsability and execution by mapping source intent to target-dialect constraints.
...
\begin{table}[ht]
\centering % <--- 让表格居中
\caption{Valid SQL features in transferred seeds. F, DT, and K denote functions, data types, and keywords; ``-'' denotes unsupported dialects. Bold marks the best result; the last row reports improvement over \sedar{}.}
\label{tab:diversity}
{\footnotesize
...
%As illustrated in Figure~\ref{fig:ablation_analysis}, 同时关闭\textit{Syntax-Aware Query Abstraction} and \textit{AQT-Guided Dialect Instantiation}， 语法正确率为50.7%，语义正确率为27.4%。
...
RISE is designed for semantic-equivalent SELECT statement translation across RDBMSs.
It decomposes a single SELECT statement into skeletons, isolates dialect-specific constructs, and asks LLMs to infer translation rules.
In contrast, \tool{} targets test-case-level seed transfer for DBMS fuzzing: AQTs model schema dependencies, nested structures, clause-level relationships, and fuzzer-compatibility constraints across full test cases.
Thus, rules in \tool{} are local AQT-node instantiation operators rather than the central translation abstraction.
...
In contrast, \tool{} reformulates transfer as AQT-Guided Dialect Instantiation.
...
% By establishing the \textit{Abstraction-then-Instantiation} paradigm, \tool{} effectively decouples logical intent from syntactic representation. The system orchestrates a static analysis engine to extract dialect-agnostic AQT, employs an AQT-guided instantiation agent to recursively instantiate these templates into target dialects, and utilizes an adaptive sanitizer to ensure seamless integration with downstream fuzzers.
