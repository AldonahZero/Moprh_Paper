...
One intuitive solution is to leverage existing SQL translation techniques~\cite{sqlglot,jooq, xie2026rise} such as SQLGlot and RISE. 
...
LLMs are only used for bounded rule generation or local repair under structural constraints.
...
(3) Finally, it adapts generated test cases to satisfy fuzzing requirements using runtime feedback, converting unsupported constructs into parsable forms while preserving their execution behavior as much as possible.
...
% - translation phase：base -> abs -> rag -> full，分别对应 direct LLM、AQT skeleton、Dialect-Aware AQT Instantiation、完整生成。
...
To further isolate test-case-level dependency modeling, we introduce $\tool{}_{\textit{w/o-ctx}}$, which disables cross-statement context propagation during AQT instantiation while keeping the same statement-level instantiation and local dialect mappings as $\tool{}_{\textit{full}}$.
Under the same \pg{}-to-\duckdb{} transfer setting as Figure~\ref{fig:ablation_analysis}, $\tool{}_{\textit{w/o-ctx}}$ achieves 93.5\% syntactic correctness, only 1.7 percentage points lower than $\tool{}_{\textit{full}}$.
However, its semantic correctness drops to 52.4\%, decreasing by 12.7 percentage points (19.5\%) from $\tool{}_{\textit{full}}$.
The main failures are reference, type, and state errors, whose rates increase from 6.8\%, 8.2\%, and 2.6\% in $\tool{}_{\textit{full}}$ to 19.5\%, 14.5\%, and 8.4\% without context propagation.
These results show that later DML/DQL statements often depend on schema objects, column types, and object states created or modified by earlier DDL/DML statements, making the evolving target context necessary for preserving executable test-case-level semantics.
