...
The \partmark entries indicate restricted, fragment-level support.
Specifically, they do not cover dependency-sensitive seed-transfer cases where DDL creates schema state later used by DML/DQL, DML relies on schema-specific behaviors such as default values or auto-increment columns, \texttt{ALTER}/\texttt{CREATE INDEX} changes the object state before later queries, or the migrated DDL--DML--DQL sequence must remain parsable and mutable by a downstream fuzzer.

RISE~\cite{xie2026rise} illustrates this boundary.
During test-case migration from PostgreSQL to a target DBMS, a source test case may contain DDL with dialect-specific schema features, DML that relies on those schema features to populate state, and a final DQL statement that uses PostgreSQL-specific date/interval expressions.
