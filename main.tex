...
To address this compatibility issue, prior work has explored SQL dialect translation for transferring SQL statements across DBMSs~\cite{zhou2020database}.
% SQL dialect translation, which transfers SQL statements across DBMS dialects~\cite{zhou2020database}.
General-purpose translators such as SQLGlot~\cite{sqlglot} and jOOQ~\cite{jooq}, and RISE~\cite{xie2026rise} mainly focus on statement-level translation.
% , mainly focus on statement-level translation for query migration or semantic-equivalent SELECT rewriting.
However, DBMS fuzzing seeds are full test cases rather than isolated statements: they often contain multiple interdependent statements including DDL, DML, and DQL.
...
It follows an LLM-centric \textit{Translate-and-Sanitize} pipeline: it first executes existing test cases on the source DBMS to collect schema information, then directly feeds the schema and raw SQL text to an LLM for target-dialect translation. To ensure compatibility with fuzzers, \sedar{} relies on \textit{Unparsable Commenting}, which temporarily comments out unparsable parts during mutation.
...
% - 注意第二个组件统一叫 Dialect-Aware AQT Instantiation；Rule Generator 保持内部 skill 名称。
...
\sedar{}~\cite{fu2024sedar}, the current state-of-the-art LLM-assisted seed transfer approach, adopts a \textit{Translate-and-Sanitize} paradigm: using an LLM to translate seeds between different dialects, followed by \textit{Unparsable Commenting} to handle parsing errors by commenting out incompatible parts or statements.
...
% \begin{definition}[Abstract Query Template]\label{def:aqt}
Given a SQL test case $\mathcal{C}$, its AQT is represented as the
five-tuple:
\[
    \mathcal{T}=\langle \mathbf{U}, \Delta, \mathcal{D}, \mathcal{G}_{tc}, \mathcal{H} \rangle .
\]
...
% Recent learning- or reduction-based systems reduce this manual effort: CrackSQL~\cite{zhou2025cracking} studies LLM-based dialect translation, RISE~\cite{xie2026rise} derives rules through dialect-aware query reduction, and QTRAN~\cite{lin2025qtran} extends metamorphic-oracle based testing across DBMS dialects.
...
It translates a query by simplifying a complex query $Q_c$ into $Q_s$, extracting a local rule $r_d$, and applying it back to $Q_c$.
However, RISE still treats a single query as the translation unit.
% In a source seed to be migrated, the final DQL statement may depend on schema features and data state created by earlier DDL/DML statements; translating the query alone does not guarantee a valid seed if this prefix is not migrated consistently.
...
% \tool{} addresses this by making the whole seed sequence the transferred unit: unlike RISE's reduced query, which isolates one dialect element within a query, \tool{}'s skeleton is the normalized per-statement form $S^{i}_{norm}$ inside an AQT that also records cross-statement dependencies $\Delta$ and evolving schema context $\mathcal{D}$.
% \tool{} addresses this by making the whole seed sequence the transferred unit: unlike RISE's reduced query, which isolates one dialect element within a query, \tool{}'s skeleton is the normalized per-statement form inside an AQT that also records cross-statement dependencies and evolving schema context.
...
Table~\ref{tab:translation-capability} summarizes the above seed-generation and translation systems along five capability dimensions relevant to fuzzing-seed transfer.
The entries marked \partmark indicate restricted support, such as limited DDL/DML translation or fragment-level handling.
Although \sedar{} supports test-case migration through LLM-based textual translation and post-hoc sanitization, it does not explicitly preserve cross-statement dependencies during migration.
More broadly, existing systems do not support dependency-preserving whole-test-case migration scenarios where: (i) DDL-created schema state must be preserved for later DML/DQL statements; (ii) DML depends on schema-specific behaviors such as default values or auto-increment columns; (iii) \texttt{ALTER TABLE} or \texttt{CREATE INDEX} changes object state before later queries; or (iv) the migrated DDL--DML--DQL sequence must remain parsable and mutable by a downstream fuzzer.
Overall, these systems improve query migration, semantic-equivalent translation, seed transfer, or oracle portability, but their goal is not dependency-preserving test-case-level seed transfer.
\begin{table}[H]
\centering
\caption{Capability comparison for fuzzing-seed transfer.}
...
\vspace{0.2em}
\parbox{\columnwidth}{\scriptsize\raggedright\textit Cross-stmt. Deps. denotes explicit preservation of schema, data, and state dependencies across statements. \cmark: explicit support; \partmark: limited support; \xmark: not supported.}
\end{table}
