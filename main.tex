...
First, \tool{} extracts an Abstract Query Template (AQT) that preserves query structure and semantics.
Second, \tool{} instantiates the AQT into a target-dialect query using verified rules.
...
To overcome this limitation, prior studies use large language models (LLMs) to translate SQL test cases across DBMSs~\cite{zhou2020database, xia2024fuzz4all, deng2023large, fu2024sedar}.
They migrate mature DBMS test cases to target systems as initial fuzzing seeds~\cite{mundler2025type, lin2024sqless}.
...
(2) Second, to make generated queries parseable by fuzzers, these methods often discard unsupported grammar constructs. This removes valuable SQL features and testing logic from the original test cases. As a result, the transferred queries may remain syntactically valid but become semantically degraded, limiting execution-path coverage and reducing the effectiveness of optimizer stress testing.
...
We evaluate \tool{} against the state-of-the-art approach \sedar{} on four popular DBMSs: \duckdb{}, \clickhouse{}, \monetdb{}, and \virtuoso{}.
\tool{} achieves \translateratio{} higher semantic correctness for transferred seeds, covers 7\%--26\% more branches, and finds 1--11 more bugs than \sedar{}.
...
To evaluate generalization across diverse architectures, we selected four widely used DBMSs according to the DB-Engines Ranking~\cite{dbengines}: MonetDB~\cite{monetdb} v11.51, Virtuoso~\cite{virtuoso} v7.2, DuckDB~\cite{duckdb_git} v1.1.3, and ClickHouse~\cite{clickhouse} v24.8 LTS.
These systems range from embedded OLAP engines to distributed columnar stores. Although they share the relational data model, each implements a distinct SQL dialect.
