...
One intuitive solution is to leverage traditional SQL translation techniques (such as SQLGlot~\cite{sqlglot}, jOOQ~\cite{jooq}, and RISE~\cite{xie2026rise}). However, these tools primarily operate at the statement level. Since DBMS fuzzing seeds are complete test cases containing multiple interdependent statements (e.g., DDL, DML, and DQL) rather than isolated clauses, statement-level translation may disrupt cross-statement dependencies (e.g., breaking schema references or type constraints), causing the entire transferred test case to fail during execution.
To bypass this limitation, recent fuzzing-oriented approaches, such as \sedar{}~\cite{fu2024sedar}, employ LLMs for end-to-end test-case transfer. For instance, \sedar{} extracts schema information from the source DBMS and feeds it alongside raw SQL text into an LLM to generate target-dialect seeds. 
...
these systems span query migration (SQLGlot, jOOQ), LLM-assisted dialect translation (CrackSQL), dialect-aware rule reduction (RISE), and metamorphic-oracle portability (QTRAN), yet none of them targets fuzzing-oriented seed construction. Their translation units remain individual statements or queries rather than complete test cases, and they do not explicitly model the cross-statement dependencies, evolving schema context, or fuzzer-grammar compatibility that whole-test-case seed transfer requires. \tool{} is designed specifically to fill this gap.
