...
The quality and diversity of initial seeds are critical for effective DBMS fuzzing.
Due to the lack of built-in test suites in many DBMSs (especially emerging databases), developers often reuse test cases from mature systems as initial seeds.
To address the SQL dialect incompatibilities, recent approaches have shifted from SQL translation to LLM-based end-to-end transfer.
Nevertheless, traditional SQL dialect translation techniques primarily operate at statement-level translation and are not designed for complete test-case transfer for DBMS fuzzing.
Recent fuzzing-oriented approaches instead employ LLMs for end-to-end test-case transfer.
However, limited knowledge of SQL structure and dialects often introduces syntactic and semantic errors, while direct transfer may simplify SQL structures and cross-statement dependencies, reducing the diversity of generated seeds.
...
To overcome this limitation, a natural direction is SQL dialect translation, which transfers SQL statements across DBMS dialects~\cite{zhou2020database}.
...
We formulate dialect translation as a two-level instantiation problem: the outer level walks the statement dependency graph while preserving the original sequence for independent statements, and the inner level recursively instantiates each statement-level graph $\mathcal{G}^i$ in a bottom-up manner.
