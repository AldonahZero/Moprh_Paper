...
One intuitive solution is to leverage existing SQL translation techniques (such as SQLGlot~\cite{sqlglot}, jOOQ~\cite{jooq}, and RISE~\cite{xie2026rise}). 
However, these tools primarily operate at the statement level. Since DBMS fuzzing seeds are complete test cases containing multiple interdependent statements rather than isolated clauses, statement-level translation may disrupt cross-statement dependencies (e.g., breaking schema references or type constraints), causing the entire transferred test case to fail during execution.
...
A typical example is \sedar{}~\cite{fu2024sedar}, which extracts schema information from the source DBMS and feeds it alongside raw SQL text into an LLM to generate target-dialect seeds. 
To handle grammar mismatches with downstream fuzzers, it temporarily comments out unparsable parts in SQL text during mutation.
