...
% Reviewer 修改逻辑总览：
% - Reviewer B/Q1 novelty vs. SedaR：Introduction、Approach overview、Related Work 统一强调 SedaR 是 LLM text translation + destructive sanitization，Morph 是 test-case-level AQT-Guided Dialect Instantiation + feedback repair，并用 RQ3/RQ2 数字支撑差异。
...
% - Reviewer B/C extensibility/manual effort：Discussion 中用 manual effort 表说明规则主要自动抽取，人工只 review/correction 少量 unmapped constructs；正文不强调 SQLServer/OpenTenBase bug 数。
...
% - Paragraph 5：提出核心思想，从 direct SQL-text translation 转向 test-case-level AQT-Guided Dialect Instantiation。
...
% 回应逻辑：把 novelty 放在 test-case-level AQT-Guided Dialect Instantiation，强调 AQT 记录 schema/nesting/clause 关系，rules 只是局部节点实例化算子。
% - 核心是任务表述的变化：从 direct SQL-text translation 转到 test-case-level AQT instantiation。
...
% - 和 SedaR/RISE 的区别要落在“test-case-level AQT-guided transfer”，少用“规则更多”这种解释。
...
% - Reviewer A：rules 只是局部 AQT-node instantiation operator；核心方法是 test-case-level AQT-Guided Dialect Instantiation，区别于 RISE 的 SELECT statement-level translation。
...
% - SQLServer/OpenTenBase 只用于 generality 和 manual effort 证据，不在正文强调 bug 数。
% LLM 参数写作逻辑：
...
% Review A-Soundness2/A-Q2 回应位置：表格列出六个 DBMS 的 auto rules、manual corrections、ratio，并补 SQLServer/OpenTenBase validity。
...
% - 表格给六个 DBMS 的人工修正比例，说明人工只占少量。
% - SQLServer/OpenTenBase 用 syntax/semantic correctness 证明 generality；这里不写新增 bug 数。
...
% 回应逻辑：先说明 DBMS fuzzing 工具解决 seed 使用和生成，再说明 dialect translators 面向 statement/query migration，最后落到 Morph 的 test-case-level seed transfer。
% - DBMS Fuzzing：说明现有 generation/mutation fuzzers 关注如何生成或变异 query，但没有解决 emerging DBMS 的 high-quality seed acquisition。
% - LLM-Based Seed Generation：SQLGlot/jOOQ/RISE/SedaR 都放在这里，维持当前小节结构。
% - 对 RISE 强调 SELECT statement-level semantic-equivalent translation；对 SedaR 强调 LLM text translation + destructive sanitization。
% - 结尾用 RQ3/RQ2 数字支撑 conceptual difference，让差异有实验落点。
...
\noindent\textbf{Data Availability.} \url{https://doi.org/10.5281/zenodo.19251660}. 
