...
\end{table}
The \partmark entries indicate that a tool may handle the corresponding SQL category in restricted forms, but not as a full dependency-preserving seed-transfer capability.
For example, during test-case migration from PostgreSQL to a target DBMS, a source test case may contain DDL that creates a table with dialect-specific schema features such as an auto-increment column, DML that relies on default-value insertion to populate that table, and a final DQL statement that filters the inserted rows using PostgreSQL-specific date/interval expressions.
RISE~\cite{xie2026rise} can apply dialect-aware query reduction to the final DQL statement: given a complex query $Q_c$ with a dialect-specific element, it obtains a simplified query $Q_s$, translates $Q_s$, extracts a local rule $r_d$, and applies the rule back to $Q_c$.
However, translating only the query-side dialect element is insufficient for test-case migration.
If the DDL prefix is not rewritten into a target-supported schema definition, or the DML prefix is dropped or rewritten inconsistently, the final DQL statement either references missing or ill-typed state or executes over a different data state.
Thus, even a correctly translated query fragment may not yield a valid, mutable fuzzing seed unless the DDL, DML, and DQL statements are migrated together with their dependencies.
...
In \tool{}, the skeleton is not a reduced query for isolating one dialect element; it is the normalized per-statement form $S^{i}_{norm}$ inside an AQT built for an ordered seed sequence.
