...
\begin{tabular}{lccccc}
    ...
    \sedar{}~\cite{fu2024sedar} & \partmark & \partmark & \cmark & \xmark & \xmark \\
...
\vspace{0.2em}
\parbox{\columnwidth}{\scriptsize\raggedright\textit{Note.} Cross-stmt. Deps. denotes cross-statement dependency preservation. \cmark: explicit support; \partmark: limited support; \xmark: not supported.}
\end{table}
...
The entries marked \partmark indicate restricted, fragment-level support.
None of these systems targets dependency-preserving whole-test-case seed transfer; specifically, they do not support the following test-case-level migration scenarios: (i) DDL-created schema state must be preserved for later DML/DQL statements; (ii) DML depends on schema-specific behaviors such as default values or auto-increment columns; (iii) \texttt{ALTER TABLE} or \texttt{CREATE INDEX} changes object state before later queries; or (iv) the migrated DDL--DML--DQL sequence must remain parsable and mutable by a downstream fuzzer.
