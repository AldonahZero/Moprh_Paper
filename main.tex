...
\[
    \mathcal{T}=\langle \mathbf{U}, \Delta, \mathcal{D}, \mathcal{G}_{tc}, \mathcal{H} \rangle .
\]
Here, $\mathbf{U}=\langle \mathcal{U}_1,\ldots,\mathcal{U}_n\rangle$ is the
ordered sequence of statement templates, where each
$\mathcal{U}_i=\langle kind_i,S^i_{norm},\mathcal{G}^i,R_i,W_i\rangle$
records the statement type, normalized skeleton, statement-level
intra-statement graph, read set, and write set of $s_i$.
$\Delta \subseteq \{(i,j)\mid i<j\}$ captures cross-statement dependencies,
where $(i,j)\in\Delta$ iff $R_j\cap W_i\neq\emptyset$.
$\mathcal{D}$ records the cumulative schema context after executing
$\langle s_1,\ldots,s_n\rangle$, including table definitions, column types,
constraints, and object-state summaries that are propagated as source-side
constraints during target-side instantiation.
$\mathcal{G}_{tc}=(V_{tc},E_{tc})$ is the test-case dependency graph, where
$V_{tc}=\{1,\ldots,n\}$ and $E_{tc}=\Delta$; thus, $\Delta$ is the dependency
set and $\mathcal{G}_{tc}$ is its graph representation for dependency-preserving
statement ordering.
Finally, $\mathcal{H}$ is a structural hash over the ordered normalized
skeletons and dependency edges, acting as a fingerprint for deduplication and
cache indexing.
% \end{definition}
