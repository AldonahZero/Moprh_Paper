...
\title{Morph: Structure-Aware Cross-Dialect SQL Transfer for Constructing High-Quality Seeds in DBMS Fuzzing}
...
Our core idea is to \textbf{reformulate SQL transfer from direct SQL-text translation into AQT-Guided Dialect Instantiation for whole test cases}.
...
\textbf{Formal Definition.} To support syntax-aware query abstraction and decouple the logical intent of a test case, we introduce SQL abstraction rules, test-case dependencies, and AQT. 
...
\item
    \vspace{-0.5cm}
    \begin{definition}[SQL Test Case]\label{def:testcase}
    A SQL test case $\mathcal{C}$ is an ordered sequence of SQL statements:
    $$\mathcal{C} = \langle s_1, s_2, \ldots, s_n \rangle$$
    where each $s_i \in \{\textit{DDL}, \textit{DML}, \textit{Query}\}$.
    Let $R_i$ denote the set of database objects read or referenced by $s_i$, and let $W_i$ denote the set of database objects whose schema or instance state is created, altered, or populated by $s_i$.
    A \textit{cross-statement dependency} exists from $s_i$ to $s_j$ ($i < j$), written $s_i \xrightarrow{dep} s_j$, iff $R_j \cap W_i \neq \emptyset$.
    The dependency set is $\Delta(\mathcal{C}) = \{(i,j) \mid i < j \land R_j \cap W_i \neq \emptyset\}$.
    \end{definition}
\item
    \vspace{-0.5cm}
    ...
    \begin{itemize}
        \item $\mathbf{U} = \langle \mathcal{U}_1, \ldots, \mathcal{U}_n \rangle$ is the ordered sequence of statement templates. Each $\mathcal{U}_i = \langle kind_i, S^i_{norm}, \mathcal{G}^i, R_i, W_i \rangle$ records the statement type, normalized skeleton, intra-statement structure graph, read/reference set, and write/effect set of $s_i$.
...
Unlike prior statement-level translators that process each SQL statement in isolation, \tool{} treats a test case as the abstraction unit. 
...
All tools were executed under identical experimental settings for 24 hours to ensure a fair comparison; therefore, the bug count in this RQ is a controlled 24-hour comparison, not the full real-world bug count reported in RQ1.
