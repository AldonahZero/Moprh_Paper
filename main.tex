...
By separating semantic-rich query structures from dialect-specific syntax, test cases can be translated across DBMSs while retaining the features that make them useful fuzzing seeds.
This enables fuzzing to be more accurate and comprehensive, as it preserves schema dependencies, type constraints, functions, and nested structures while avoiding translation failures caused by superficial syntactic differences.
...
\begin{table}[ht]
\centering % <--- 让表格居中
\scriptsize     % <--- 主动缩小字号，而不是被拉伸
\caption{Statistics of valid SQL features in the transferred seeds. F, DT, and K denote functions, data types, and keywords, respectively. The best result in each column is highlighted in bold.}
\label{tab:diversity}
...
Recent LLM-based methods improve DBMS seed generation: CrackSQL~\cite{zhou2025cracking}, RISE~\cite{xie2026rise}, and Fuzz4All~\cite{xia2024fuzz4all} improve seed variety, while \sedar{}~\cite{fu2024sedar}, ShQveL~\cite{zhong2025testing}, and QTRAN~\cite{lin2025qtran} explore cross-dialect transfer, fragment synthesis, and oracle extension. Unlike them, \tool{} targets fuzzer-ready crash-bug seeds and preserves SQL feature diversity across dialects.
