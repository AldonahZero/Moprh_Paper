...
However, existing SQL dialect translation methods are primarily designed for DQL translation and struggle to handle entire test cases that contain interdependent statements, including DDL, DML, and DQL.
Recent fuzzing-oriented methods employ LLMs to perform end-to-end transfer across DBMSs. Nevertheless, due to hallucination and limited structure and dialect knowledge, such methods often introduce syntax and semantic errors and produce fuzzer-incompatible outputs.
...
During this parsing, it constructs $\mathcal{G}^i$ by creating nodes for scoped AST fragments such as query blocks, CTE bodies, and nested subqueries, and edges for containment and fragment-dependency relations needed by bottom-up instantiation.
It also returns read/write sets $(R_i,W_i)$, which are used to add dependency edges from earlier statements (Line~\ref{line:alg1:dep}).
Here, $Ctx$ is the mutable extraction-time context: Algorithm~\ref{alg:aqt_extraction} updates it after each statement and stores its final snapshot as $\mathcal{D}$, while Algorithm~\ref{alg:translation} initializes the source-side constraint context $C_s$ from $\mathcal{D}$ and evolves only the target context $C_t$.
After the loop completes, \tool{} snapshots the evolved context as $\mathcal{D}$, constructs the test-case dependency graph $\mathcal{G}_{tc}$, computes the structural hash $\mathcal{H}$, and returns the AQT five-tuple (Lines~\ref{line:alg1:finalize_d}--\ref{line:alg1:return}).
...
$\mathcal{G}_{tc} \leftarrow (\{1,\ldots,|\mathbf{U}|\}, \Delta)$ \label{line:alg1:finalize_gtc}\;
$\mathcal{H} \leftarrow \textsc{SHA256}(\bigoplus_i \textsc{Skeleton}(\mathcal{U}_i) \oplus \Delta)$ \label{line:alg1:finalize_h}\;
...
$\textsc{IndexArtifact}((\mathcal{H}, D_t), \mathcal{Q}_{target})$ \label{line:alg2:index} \tcp*{Dedup/cache key outside recursion}
...
The algorithm derives a stable dependency order over $\mathcal{G}_{tc}$, the graph representation of $\Delta$, which respects cross-statement dependencies while preserving the source order for independent statements (Line~\ref{line:alg2:tc_sort}).
For each statement, it invokes \textsc{InstantiateStmt} with source-side constraints carried by $\mathcal{D}$ and the current target context, and then propagates the updated target context to subsequent statements (Lines~\ref{line:alg2:stmt_inst}--\ref{line:alg2:schema_prop}).
After all statements are instantiated, $\mathcal{H}$ is paired with the target dialect $D_t$ to deduplicate structurally equivalent AQTs and index cached results (Line~\ref{line:alg2:index}).
Inside \textsc{InstantiateStmt}, \tool{} follows the statement-level intra-statement graph $\mathcal{G}^i$ in bottom-up order, substitutes instantiated child fragments, retrieves or generates local dialect mappings from the Hybrid KB, and validates the resulting fragments with the target DBMS parser before caching new rules. If validation fails, the Error Fixer repairs the fragment and rule using parser feedback; unverified candidates are discarded rather than reused. This mechanism avoids degenerate fixes such as clause removal and preserves testing-relevant SQL features.
