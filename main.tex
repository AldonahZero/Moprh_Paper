...
It then performs AQT-Guided Dialect Instantiation to construct a target seed sequence under target-dialect constraints.
...
To guarantee that the generated queries remain semantically consistent with the target dialect during test-case instantiation, the system employs an AQT-Guided Dialect Instantiation process as formalized in Algorithm~\ref{alg:translation}.
...
\textit{SQL Translator:} This skill instantiates a token into a dialect-compliant fragment by integrating the input segment with function signatures retrieved from the \textit{Vector Index}. For example, it maps $<$\texttt{TimeDiff}$>$ to \duckdb{}'s \texttt{date\_diff} signature.
...
% 上一版（仍可再精简，保留备查）：
% \textbf{Walk-Through Example.}
% As shown in Figure~\ref{fig:agent_translation}, since $id_1$ depends on the nested segment $id_2$ ($Order=[id_2 \rightarrow id_1]$), \tool{} first instantiates the inner subquery $id_2$---mapping \texttt{<AggMin>} to \texttt{MIN(..)} and \texttt{<TypeCast>} to a dialect-valid \texttt{CAST(.. AS ..)}---and validates it via parser feedback. It then substitutes the validated fragment into the outer predicate of $id_1$, mapping $<$\texttt{TimeDiff}$>$ to DuckDB's \texttt{date\_diff}, yielding a \textit{Compatible Seed} that preserves the nested time-window logic.
\textbf{Walk-Through Example.}
As shown in Figure~\ref{fig:agent_translation}, following the order $[id_2 \rightarrow id_1]$, \tool{} first instantiates the inner subquery $id_2$ (e.g., \texttt{<AggMin>}$\rightarrow$\texttt{MIN(..)}, \texttt{<TypeCast>}$\rightarrow$\texttt{CAST(.. AS ..)}) and validates it, then substitutes it into $id_1$'s outer predicate (\texttt{<TimeDiff>}$\rightarrow$\texttt{date\_diff}), yielding a \textit{Compatible Seed} that preserves the nested time-window logic.
...
% 段落写作逻辑（本小节分四段，避免一大段堆砌）：
% 1) 挑战段：fuzzer 支持的 grammar 通常小于 DBMS，要把高级特征适配成可解析/可变异形式，且保语义。
% 2) 思路+流程段：用 parsing/execution feedback 做 structure-preserving 适配；对应 Figure 5 工作流。
% 3) Error-Guided Sanitization：定位 unsupported construct 到 AQT 节点并按语法反馈修复（去括号例子）。
% 4) Tiered Fallback：四级兜底顺序，强调 comment out 是最后手段（区别 SedaR）。
% 5) Mutable-Region Annotation：通过 parser 后标记 literal/predicate 为可变区，引导变异。
% 挑战段：点出 fuzzer grammar < DBMS 的根本矛盾，naive comment out 会破坏结构与测试价值。
% 旧版（段落过长，且 figure 浮动后容易把前后文字视觉上连成一段，保留备查）：
...
Some DBMS features exceed the grammar supported by existing fuzzers, making otherwise valid target seeds difficult to mutate.
...
\smallskip
\noindent\textbf{Feedback-Guided Adaptation.}\par
...
% Error-Guided Sanitization 段：定位 unsupported construct 到 AQT 节点并按语法反馈修复（去括号例子）。
\smallskip
\noindent\textbf{Error-Guided Sanitization.}\par
% 旧版（偏长，保留备查）：
...
% Tiered Fallback 段：四级兜底顺序，强调 comment out 是最后手段（区别 SedaR）。
% Review C-Presentation 回应位置：error message example 用完整的 ``SELECT''，处理原稿 ``SELEC'' typo。
\smallskip
\noindent\textbf{Tiered Fallback Strategy.}\par
% 旧版（过长，保留备查）：
...
% Mutable-Region Annotation 段：通过 parser 后标记 literal/predicate 为可变区，引导变异。
\smallskip
\noindent\textbf{Mutable-Region Annotation.}\par
% 旧版（偏长，保留备查）：
