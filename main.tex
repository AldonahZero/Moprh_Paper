...
Table~\ref{tab:translation-capability} summarizes seed-generation and translation systems along capability dimensions that are directly relevant to fuzzing-seed transfer.
General-purpose translators such as SQLGlot and jOOQ, together with RISE, CrackSQL, and QTRAN, primarily target statement-level or fragment-level translation.
Even \sedar{}, which supports test-case migration, does not explicitly preserve cross-statement dependencies during migration.
Consequently, no prior system simultaneously supports test-case migration and cross-statement dependency preservation.
Since \sedar{} provides the strongest existing baseline in this capability matrix, the following motivating example uses \sedar{} to illustrate the practical consequence of this gap rather than to single out an isolated weakness of one tool.
...
Because \sedar{} is the strongest existing seed-transfer baseline in Table~\ref{tab:translation-capability}, its behavior provides a representative stress case for this systematic capability gap.
...
As summarized in Table~\ref{tab:translation-capability} in Section~\ref{sec:background}, these systems improve query migration, semantic-equivalent translation, seed transfer, or oracle portability, but they are not designed for fuzzing-oriented, dependency-preserving whole-test-case transfer.
