...
Despite being able to transform test cases from mature DBMSs into initial seeds for target DBMSs, these methods still suffer from 
...
(2) Second, \tool{} performs Dialect-Aware AQT Instantiation, recursively instantiating AQT nodes into dialect-specific subtrees under target-dialect constraints while propagating the evolving target schema context across statements.
...
% - outer level：先做 dependency-preserving + 保序的拓扑排序，再逐语句 InstantiateStmt 并传播 target context，最后整体 index 复用。
% - inner level：InstantiateStmt 在语句内递归，用检索/生成的局部映射 + parser feedback 保证可执行。

\textbf{Two-Level Instantiation.}
...
Here, semantic correctness is defined by executability in the target DBMS without runtime errors, not by result equivalence between the source and target DBMSs. We separately measure semantic richness by counting valid functions, data types, and keywords preserved in executable transferred seeds.
