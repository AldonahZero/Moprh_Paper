...
Given a SQL test case $\mathcal{C}$, its AQT is represented as:
\[
    \mathcal{T}=\langle \mathbf{U}, \Delta, \mathcal{D}, \mathcal{H} \rangle .
\]
...
records the statement type, normalized skeleton, intra-statement structure,
read set, and write set of $s_i$.
...
constraints, and object-state summaries needed for target-side instantiation.
Finally, $\mathcal{H}$ is a structural hash computed over the ordered
normalized skeletons and dependency edges for deduplication.
