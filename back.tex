...
(ii) Second, a \textit{Test-Oriented Skill Executor} 
Our Skill Executor synthesizes concrete SQL fragments by retrieving and incorporating relevant context from the hybrid KB, drawing on patterns similar to those in modern agent extension frameworks~\cite{anthropic-skills-2025}.
...
As shown in center panel of Figure~\ref{fig:agent_translation}, the \textit{Skill Executor} acts as the reasoning core of the agent, equipped with a suite of specialized skills to handle the instantiation.
...
% CrackSQL~\cite{zhou2025cracking} uses genetic algorithms with LLMs to mine mutation patterns, RISE~\cite{xie2026rise} employs skeleton-based query decomposition, and Fuzz4All~\cite{xia2024fuzz4all} applies LLM prompting across domains.
