...
\node[right=0.5cm of graphNode, font=\scriptsize\sffamily, color=blue] (hcLabels) {
    \textbf{$\mathcal{H}$}: \texttt{0x3f7a} \textcolor{gray}{\textit{(hash)}} \quad
    \textbf{$\mathcal{C}$}: 8.5 \textcolor{gray}{\textit{(nodes+depth)}}
};
...
\node[dialogBox] (step3) at (aqtContainer.south |- bottomY) {
    ...
    Creates matchable query representation Tuple $\mathcal{T} = \langle S_{norm}, \mathcal{G}, \mathcal{D}, \mathcal{H}, \mathcal{C} \rangle$
};
